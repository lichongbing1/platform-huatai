...
import com.platform.utils.SessionUtils;
...
public class SysSmsLogServiceImpl implements SysSmsLogService {
    ...
    public SysSmsLogEntity sendSms(SysSmsLogEntity smsLog) {
        ...
        if (StringUtils.isNullOrEmpty(smsLog.getUserId())) {
            smsLog.setReturnMsg("系统用户：" + smsLog.getReturnMsg());
            smsLog.setUserId(SessionUtils.getUserId());
        }
        if (smsLog.getUserId() == 1) {
            smsLog.setReturnMsg("外部系统：" + smsLog.getReturnMsg());
        }
